...
parser_error_t _readOptionMultiSignature_V2(parser_context_t* c, pd_OptionMultiSignature_V2_t* v)
{
    CHECK_ERROR(_readUInt8(c, &v->some))
    if (v->some > 0) {
        CHECK_ERROR(_readMultiSignature_V2(c, &v->contained))
    }
    return parser_ok;
}
...
parser_error_t _readOptionMultiSigner_V2(parser_context_t* c, pd_OptionMultiSigner_V2_t* v)
{
    CHECK_ERROR(_readUInt8(c, &v->some))
    if (v->some > 0) {
        CHECK_ERROR(_readMultiSigner_V2(c, &v->contained))
    }
    return parser_ok;
}
...
parser_error_t _readOptionTupleBalanceOfTBalanceOfTBlockNumber_V2(parser_context_t* c, pd_OptionTupleBalanceOfTBalanceOfTBlockNumber_V2_t* v)
{
    CHECK_ERROR(_readUInt8(c, &v->some))
    if (v->some > 0) {
        CHECK_ERROR(_readTupleBalanceOfTBalanceOfTBlockNumber_V2(c, &v->contained))
    }
    return parser_ok;
}
...
parser_error_t _readOptionXcmVersion_V2(parser_context_t* c, pd_OptionXcmVersion_V2_t* v)
{
    CHECK_ERROR(_readUInt8(c, &v->some))
    if (v->some > 0) {
        CHECK_ERROR(_readXcmVersion_V2(c, &v->contained))
    }
    return parser_ok;
}
    ...
    uint8_t* pageCount)
{
    ...
    *pageCount = 1;
    if (v->some > 0) {
        CHECK_ERROR(_toStringMultiSignature_V2(
            &v->contained,
            outValue, outValueLen,
            pageIdx, pageCount));
    } else {
    ...
    uint8_t* pageCount)
{
    ...
    *pageCount = 1;
    if (v->some > 0) {
        CHECK_ERROR(_toStringMultiSigner_V2(
            &v->contained,
            outValue, outValueLen,
            pageIdx, pageCount));
    } else {
    ...
    uint8_t* pageCount)
{
    ...
    *pageCount = 1;
    if (v->some > 0) {
        CHECK_ERROR(_toStringTupleBalanceOfTBalanceOfTBlockNumber_V2(
            &v->contained,
            outValue, outValueLen,
            pageIdx, pageCount));
    } else {
    ...
    uint8_t* pageCount)
{
    ...
    *pageCount = 1;
    if (v->some > 0) {
        CHECK_ERROR(_toStringXcmVersion_V2(
            &v->contained,
            outValue, outValueLen,
            pageIdx, pageCount));
    } else {
